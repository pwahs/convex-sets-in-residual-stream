...
def kruskal(points, exclude, probabilistic=False, num_rays=100, debug=True):
    ...
    for e, edge in enumerate(edges):
        ...
        if (
            ...
            if probabilistic:
                hull = ProbabilisticPointSetWithConvexHull(
                    new_points, num_rays=num_rays
                )
            else:
                hull = PointSetWithConvexHull(new_points)
            if (
                probabilistic
                and not any(hull.is_inside_hull(point) for point in exclude)
            ) or (
                not probabilistic
                and not any(hull.is_inside_hull_2d(point) for point in exclude)
            ):
